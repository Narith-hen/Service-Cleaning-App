...
const CancelDuringWorkPage = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    const normalizedJobId = String(jobId || '').replace(/^confirmed-/, '');

    try {
      if (normalizedJobId) {
        await api.patch(`/bookings/${normalizedJobId}/status`, {
      ...
      // Keep navigation non-blocking if the API or storage update fails.
    }
