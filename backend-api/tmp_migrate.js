...
async function migrateDatabase() {
  ...
  try {
    ...
    if (tables.length > 0) {
      ...
      const hasSenderAcc = columns.some(c => c.Field === 'sender_id');
      ...
      if (!hasSenderAcc) {
        ...
        await promiseDb.query(`
          ...
          ADD COLUMN sender_id INT DEFAULT NULL AFTER service_package_id,
          ADD COLUMN receiver_id INT DEFAULT NULL AFTER sender_id
        `);

        // Migrate old messages: users
        await promiseDb.query(`
          UPDATE messages m 
        ...
        // Migrate old messages: cleaners
        await promiseDb.query(`
          UPDATE messages m 
          JOIN acc a ON m.old_sender_id = a.cleaner_id
          SET m.sender_id = a.acc_id
        `);
        await promiseDb.query(`
          UPDATE messages m 
          JOIN acc a ON m.old_receiver_id = a.cleaner_id
          SET m.receiver_id = a.acc_id
        `);
