...
const parseDatabaseUrl = (rawUrl) => {
  ...
  try {
    ...
    return {
      ...
      database: parsedUrl.pathname ? decodeURIComponent(parsedUrl.pathname.replace(/^\//, '')) : undefined
    };
  } catch (error) {
    console.warn('Invalid DATABASE_URL format. Falling back to individual DB_* environment variables.');
    return {};
  }
};
...
const isLocalDbHost = (value) => ['127.0.0.1', '::1', 'localhost'].includes(String(value || '').trim().toLowerCase());

const encodePathSegment = (value) => encodeURIComponent(String(value || '')).replace(/%2F/g, '/');
...
const getResolvedDatabaseUrl = () => {
  ...
  return `mysql://${auth}${config.host}:${config.port}/${encodePathSegment(config.database)}`;
};
